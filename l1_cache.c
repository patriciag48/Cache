                     ...
                     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  uint32_t set_index = (address & L1_SET_INDEX_MASK) >> L1_SET_INDEX_SHIFT;
  ...
  int i;
  for(i = 0; i < L1_LINES_PER_SET; i++){
    if(((l1_cache[set_index].lines[i].v_r_d_tag & L1_VBIT_MASK) != 0) && ((l1_cache[set_index].lines[i].v_r_d_tag & L1_ENTRY_TAG_MASK) == address_tag)){
      break;
    }
  }

  if(i == L1_LINES_PER_SET){
                    ...
                    uint8_t *status)
{
  ...
  for(int i = 0; i < L1_LINES_PER_SET; i++){
    if((l1_cache[set_index].lines[i].v_r_d_tag & L1_VBIT_MASK) == 0){
      ...
      for(int j = 0; j < WORDS_PER_CACHE_LINE; j++){
        l1_cache[set_index].lines[i].cache_line[j] = write_data[j];
      }
      l1_cache[set_index].lines[i].v_r_d_tag = l1_cache[set_index].lines[i].v_r_d_tag | L1_VBIT_MASK;
      l1_cache[set_index].lines[i].v_r_d_tag = l1_cache[set_index].lines[i].v_r_d_tag & ~(L1_DIRTYBIT_MASK);
      l1_cache[set_index].lines[i].v_r_d_tag = l1_cache[set_index].lines[i].v_r_d_tag & ~(L1_RBIT_MASK);
      l1_cache[set_index].lines[i].v_r_d_tag = (l1_cache[set_index].lines[i].v_r_d_tag & ~(L1_ENTRY_TAG_MASK)) | address_tag;
      *status = *status & ~(L1_CACHE_HIT_MASK);
      return;
    }
    ...
    else if((r0_d0_index == ~0x0) && ((l1_cache[set_index].lines[i].v_r_d_tag & L1_RBIT_MASK) == 0) && ((l1_cache[set_index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) == 0)){
      r0_d0_index = i;
    }
    else if((r0_d1_index == ~0x0) && ((l1_cache[set_index].lines[i].v_r_d_tag & L1_RBIT_MASK) == 0) && ((l1_cache[set_index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) != 0)){
      r0_d1_index = i;
    }
    else if((r1_d0_index == ~0x0) && ((l1_cache[set_index].lines[i].v_r_d_tag & L1_RBIT_MASK) != 0) && ((l1_cache[set_index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) == 0)){
      r1_d0_index = i;
    }
  }
  ...
  if((l1_cache[set_index].lines[index].v_r_d_tag & L1_DIRTYBIT_MASK) != 0){
    uint32_t evicted_entry_tag = l1_cache[set_index].lines[index].v_r_d_tag & L1_ENTRY_TAG_MASK;
    *evicted_writeback_address = (evicted_entry_tag << L1_ADDRESS_TAG_SHIFT) | (set_index << L1_SET_INDEX_SHIFT);
    for(int i = 0; i < WORDS_PER_CACHE_LINE; i++){
      evicted_writeback_data[i] = l1_cache[set_index].lines[index].cache_line[i];
    }
    *status = *status | L1_CACHE_HIT_MASK;
  }
  else{
  ...
  for(int i = 0; i < WORDS_PER_CACHE_LINE; i++){
    l1_cache[set_index].lines[index].cache_line[i] = write_data[i];
  }
  l1_cache[set_index].lines[index].v_r_d_tag = l1_cache[set_index].lines[index].v_r_d_tag | L1_VBIT_MASK;
  l1_cache[set_index].lines[index].v_r_d_tag = l1_cache[set_index].lines[index].v_r_d_tag & ~(L1_DIRTYBIT_MASK);
  l1_cache[set_index].lines[index].v_r_d_tag = (l1_cache[set_index].lines[index].v_r_d_tag & ~(L1_ENTRY_TAG_MASK)) | address_tag;
}
