...
/***************************************************
 ...
 entry in the L2 cache. It has the following fields:
   v_d_tag: 32-bit unsigned word containing the
            ...
            in bits 0 through 11 (the 12 rightmost bits)
                     ...
                     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  if(((l2_cache[index].v_d_tag & L2_VBIT_MASK) == 0) || ((l2_cache[index].v_d_tag & L2_ENTRY_TAG_MASK) != tag)){
                    ...
                    uint8_t *status)
{
  ...
  if(((l2_cache[index].v_d_tag & L2_VBIT_MASK) == 0) || ((l2_cache[index].v_d_tag & L2_DIRTYBIT_MASK) == 0)){
    for(int i = 0; i < WORDS_PER_CACHE_LINE; i++){
      l2_cache[index].cache_line[i] = write_data[i];
    }
