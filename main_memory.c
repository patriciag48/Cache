...

{
  ...
  if (address > main_memory_size_in_bytes) {
    printf("Error: Address is not within the size of the memory\n");
    exit(1);
  }
  ...
  j = cache_line_address;
  if(control & WRITE_ENABLE_MASK){
    for(int i = 0; i < WORDS_PER_CACHE_LINE; i++){
      main_memory[j] = write_data[i];
      j++;
    }
  }
